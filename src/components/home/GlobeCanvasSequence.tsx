...
interface GlobeCanvasSequenceProps {
  scrollYProgress: MotionValue<number>;
  frameCount?: number;
}

export default function GlobeCanvasSequence({ scrollYProgress, frameCount = 120 }: GlobeCanvasSequenceProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= frameCount; i++) {
      ...
      // Expecting files like /globe-sequence/frame-001.jpg
      img.src = `/globe-sequence/frame-${paddedIndex}.jpg`;
      img.onload = () => {
        loadedCount++;
    ...
    <div className="absolute inset-0 h-full w-full">
      {failed ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-gold-600/30">
          ...
          <p className="font-display font-bold uppercase tracking-[0.3em]">Awaiting Globe Sequence</p>
          <p className="mt-2 text-xs opacity-50">Drop frames into /public/globe-sequence/</p>
        </div>
      ) : (
        <canvas
          ref={canvasRef}
