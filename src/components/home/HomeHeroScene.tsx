...
import { heroHighlights, heroPills } from "@/data/home-content";
 ...
 * - background canvas scrubs through a 3D cinematic sequence
 * - glow blobs float away at their own rates
 * - pinned section allows the 3D scroll animation to play out completely before moving to the next section
 */
export default function HomeHeroScene() {
  ...
  // Fade out content just as we reach the end of the scroll pinning
  const contentY = useTransform(scrollYProgress, [0.8, 1], [0, -60]);
  const contentOpacity = useTransform(scrollYProgress, [0.8, 1], [1, 0]);

  return (
    <section ref={ref} className="relative isolate h-[250vh] bg-navy">
      <div className="sticky top-0 flex h-screen w-full items-center overflow-hidden text-white">
        
        {/* Background Canvas Layer */}
        <HeroCanvasSequence scrollYProgress={scrollYProgress} />
        
        {/* Darkening Gradients for readability - lightened so background is visible */}
        <div aria-hidden className="absolute inset-0 bg-gradient-to-br from-navy/30 via-navy/40 to-navy/70" />
        <div aria-hidden className="absolute inset-0 bg-gradient-to-t from-navy/60 via-transparent to-transparent" />

        {/* Middle layers: glow blobs */}
        <motion.div
          aria-hidden
        ...
        {/* Foreground Content */}
        <motion.div className="w-full" style={reduce ? undefined : { y: contentY, opacity: contentOpacity }}>
          <div className="container-page relative grid items-center gap-14 lg:grid-cols-[1.08fr_.92fr]">
            <div>
              <Reveal>
